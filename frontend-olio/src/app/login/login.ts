...
export class LoginComponent implements OnInit {
  ...
  onSubmit() {//gestisce l'autenticazione utente quando l'utente invia il form di login
    this.messaggioErrore = ''; // Pulisco eventuali messaggi di errore precedenti
    ...
    localStorage.removeItem('utente'); // Pulisco preventivamente anche il mio riferimento locale residuo da sessioni precedenti

    this.utenteService.login(this.credenziali).subscribe({//invio le credenziali al backend
      next: (response: any) => { // <-- Tipizzo la response per il mio Strict Mode
        ...
        if (token) {//se il token è valido il login viene reso effettivo su tutta la piattaforma
          // Uso salvaSessione per notificare il BehaviorSubject che avviserà in tempo reale tutti gli altri componenti di Angular
        ...
        //gestisce cosa succede subito dopo aver salvato i dati (se il login va a buon fine)
        //  e come comportarsi se invece qualcosa va storto.


        // Prima di navigare, verifico che il mio token sia effettivamente presente
        const checkToken = () => {
          if (this.utenteService.isLoggedIn()) {
             const returnUrl = this.route.snapshot.queryParams['returnUrl'] || (ruolo === 'ADMIN' ? '/login/admin' : '/');//decido dove reindirizzare l'utente in base al suo ruolo
             this.router.navigateByUrl(returnUrl);//se l'utente era nel carrello, dopo il login lo reindirizzo al checkout, altrimenti alla home
          } else {
      ...
      //se viene RIFIUTATO IL TENTATIVO DI ACCESSO:
